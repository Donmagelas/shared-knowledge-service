...
File 上传 → Docling 解析 → HybridChunker → Embedding
          ...
          → Payload Filter → Qdrant 原生 RRF → OGX Search
```

共享 Collection、逻辑 VectorStore 隔离、同一 File 多重挂载、scoped delete、失败挂载重试、Docling Worker 租约回收和三组件重启恢复均有自动化测试。`/knowledge/v1/ingest` 已实现同步上传与建索引，`/knowledge/v1/search` 已实现单个或多个逻辑知识库的一次 Qdrant 查询。真实 Embedding 与 Stella/企业版真实项目文档的第一轮评测已完成；当前仍未完成的是更大业务语料复核、Reranker、认证、S3 和备份等生产化工作。
...
自定义 Provider 在每个部署中只创建一个物理 Qdrant Collection。OGX `VectorStore` 是逻辑知识库，写入 Point 的 `vector_store_id` Payload；所有检索和删除都会由服务端强制带上该范围。业务过滤字段写入 `attributes`，需要高性能过滤的字段应在 `config/ogx.yaml` 的 `payload_indexes` 中按部署显式声明。
...
复制 `.env.example` 为 `.env` 并填写真实 Embedding 配置后启动：
...
创建逻辑知识库仍复用 OGX 辅助接口 `POST /v1/vector_stores`。Stella 通常只在部署初始化时创建一个隐藏 VectorStore；企业版为公司、部门、产品等显式业务知识库分别创建并保存映射。

同步导入在一次请求中完成原文件持久化、Docling、HybridChunker、Embedding 和 Qdrant 写入：
...
响应返回 `completed` 或 `failed`。如果上传完成后处理失败，响应仍返回已持久化的 `file_id`；调用方可通过 OGX 文件挂载辅助接口删除失败记录后显式重试，服务不会假装完整导入任务能够跨进程自动续跑。
...
`mode` 支持 `hybrid`、`dense` 和 `bm25`。多个 `knowledge_base_ids` 会转换成一条 `vector_store_id IN [...]` 的 Qdrant 查询；返回 `hits` 中每项固定包含 `file_id`、`chunk_id`、`content`、`locator`、`score` 和 `attributes`。
