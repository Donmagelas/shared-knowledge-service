...
| 对象 | 含义 | 持久化表达 |
| --- | --- | --- |
| Tenant | 一套相互隔离的模型配置和向量数据空间 | 一个独立 Qdrant Collection；物理名称由服务生成 |
| KnowledgeBase | 可上传、查询和删除的技术知识库 | OGX VectorStore；在租户 Collection 中写为 vector_store_id Payload |
...
| Admin Token | Runtime Token 的超集；另外可配置租户 Embedding 和 Rerank |

产品运行时应使用 Runtime Token；只有可信部署或租户初始化流程使用 Admin Token。OGX 原生接口不是稳定产品契约，并且只允许 Admin Token 访问。
...
| 字段 | 必填 | 约束 | 含义 |
| --- | --- | --- | --- |
| base_url | 是 | 1～2048 字符 | OpenAI-compatible Embedding 接口基址 |
| api_key | 首次必填；后续可省略 | 1～8192 字符 | 租户独立凭证；省略表示保留原凭证 |
| model_id | 是 | 1～512 字符 | 上游模型 ID |
| dimension | 是 | 1～65536 | 模型真实输出维度 |

示例：

~~~bash
# 首次配置时 api_key 必填；服务会立即探测 URL、模型和向量维度。
curl -X PUT "http://localhost:8321/knowledge/v1/tenants/stella/embedding-config" \
...
1. 租户首次 Ingest 成功创建或确认 Qdrant Collection 后，locked 变为 true；即使后续文件解析失败也不会自动解锁。
2. 锁定后不能修改 base_url、model_id 或 dimension，因为已有向量不能与新向量空间混用。
3. 锁定后仍可用相同 base_url、model_id、dimension 提交新 api_key，以轮换凭证。
4. 服务不提供模型列表，也不推断模型维度；产品或部署方必须提交准确配置。
...
| 字段 | 必填 | 含义 |
| --- | --- | --- |
| enabled | 是 | 是否在 hybrid 检索中启用远程 Rerank |
| base_url | 启用时必须存在 | OpenAI-compatible Rerank 接口基址 |
| api_key | 首次启用时必须存在；后续可省略 | 与 Embedding 相互独立的租户凭证 |
| model_id | 启用时必须存在 | Rerank 模型 ID |

启用示例：

~~~bash
# Rerank Key 与 Embedding Key 可以不同。
curl -X PUT "http://localhost:8321/knowledge/v1/tenants/stella/rerank-config" \
  -H "Authorization: Bearer <admin-token>" \
  -H "Content-Type: application/json" \
  -d '{
    "enabled": true,
...
Rerank 配置不决定持久化向量空间，因此可以按租户修改、轮换 Key 或关闭。当前只在 mode=hybrid 时执行；远程 Rerank 失败时，Search 降级返回 Qdrant RRF 结果。
...
响应字段：enabled、base_url、model_id、credential_configured、updated_at，不返回明文 api_key。
...
只有 tenant_id 是业务输入。名称、描述、公司/部门/产品归属等业务字段由产品数据库维护，不复制到技术 KnowledgeBase。
...
租户尚未配置 Embedding 时返回 409 embedding_config_required。
...
| mode | 链路 |
| --- | --- |
| dense | Query Embedding → Qdrant Dense HNSW |
| bm25 | Qdrant 原生 BM25 Sparse Query |
| hybrid | Dense + BM25 → Qdrant RRF → 租户启用时远程 Rerank |

Rerank 是租户配置，不是每次 Search 的请求参数。这样可以避免调用方绕过部署确定的质量与成本策略。Rerank 失败时服务返回未 Rerank 的 RRF 结果，不让一次外部模型故障直接中断检索。
...
服务始终把 knowledge_base_ids 翻译成强制 vector_store_id 范围，再与 filters 做 AND。调用方无法通过 Filter 越过本次允许的 KnowledgeBase 集合。
...
| 方法 | 路径 | Token | 主要用途 |
| --- | --- | --- | --- |
| PUT | /tenants/{tenant_id}/embedding-config | Admin | 配置或轮换 Embedding |
| GET | /tenants/{tenant_id}/embedding-config | Admin | 查询非敏感 Embedding 配置 |
| PUT | /tenants/{tenant_id}/rerank-config | Admin | 配置、轮换或关闭 Rerank |
| GET | /tenants/{tenant_id}/rerank-config | Admin | 查询非敏感 Rerank 配置 |
...
1. 运维用 Admin Token 配置租户 Embedding。
2. 按需配置或关闭租户 Rerank。
3. Stella 用稳定 Idempotency-Key 创建一个隐藏技术 KnowledgeBase。
4. 把 tenant_id 和 knowledge_base_id 保存在 Stella 配置或数据库中。
5. 部署 payload_indexes：scope、user_id、agent_id 使用 keyword。
...
| 场景 | 接口 |
| --- | --- |
| 部署初始化 | Embedding/Rerank Config、创建/查询隐藏 KnowledgeBase |
...
2. 用该租户自己的 Embedding URL、Key、模型 ID 和维度调用 Admin 配置接口。
3. 用该租户自己的独立 Rerank URL、Key、模型 ID 和开关调用 Admin 配置接口。
4. 同一服务会为不同 tenant_id 使用相互隔离的 Qdrant Collection。

租户凭证只保存在统一知识库的加密配置中。企业版数据库可保存配置状态或最后更新时间，但不应要求 Knowledge API 回传明文 Key。
...
企业版数据库创建业务 KnowledgeBase
  → POST /knowledge-bases 创建技术 KnowledgeBase
...
若第二步失败，企业版可以用同一个 Idempotency-Key 重试；不要重复创建业务对象。
...
用户发起检索
  ...
  → 一次 POST /search
  → 统一知识库在该租户 Collection 内执行一次过滤检索
~~~
...
| 场景 | 接口 |
| --- | --- |
| 租户开通 | Embedding/Rerank Config |
| 创建业务知识库 | 创建技术 KnowledgeBase，并保存 ID 映射 |
...
| 用户、组织、角色、Agent/Assistant | 租户技术路由和模型配置 |
...
| 决定何时创建、重试和删除 | Dense、BM25、RRF、可选 Rerank 和 Qdrant |
...
| 409 | embedding_config_required、embedding_config_locked | 租户模型未配置或向量空间已锁定 |
